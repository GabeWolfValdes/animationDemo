	...
	: GraphicsObject(model, pShaderObj)
{
	...
	Matrix ScaleUV(Matrix::Scale, w, h, 1.0f);
	Matrix TransUV(Matrix::Trans, u, v, 0.0f);
	*this->poMatrix_uv = ScaleUV * TransUV;
	...
	Matrix Scale(Matrix::Scale, this->origWidth, this->origHeight, 1.0f);
...
void GraphicsObject_Sprite::SetDataGPU()
{
	...
	// World matrix ----------------------------
	Matrix TransToOriginLowerLeft(Matrix::Trans, -pCam->getScreenWidth() / 2.0f, -pCam->getScreenHeight() / 2.0f, 0.0f);
