...
void Camera::MoveForward()
{
	...
	this->getHelper(_up, _tar, _pos, _upNorm, _forwardNorm, _rightNorm);

	_pos = _pos + 5.53f * _forwardNorm;
	_tar = _tar + 5.53f * _forwardNorm;


	this->setHelper(_up, _tar, _pos);
}
...
void Camera::RotateUp()
{
	...
	Matrix Trans(Matrix::Trans, _tar);
	Matrix NegTrans(Matrix::Trans, -_tar);
	Matrix Rot;
	Rot.set(Matrix::RotAxisAngleType::RotAxisAngle, _rightNorm, -0.03f);
...
void Camera::RotateRight()
{
	...
	Matrix Rot;
	Rot.set(Matrix::RotAxisAngleType::RotAxisAngle, _upNorm, +0.03f);
